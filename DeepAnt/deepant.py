...
class DeepAnt(nn.Module):
    def __init__(self, in_channels, seq_len):
        ...
        self.convblock1 = nn.Sequential(
            nn.Conv1d(in_channels=in_channels, out_channels=32, kernel_size=3, padding='valid'),
        ...
        conv1_output_size = (seq_len - 3) // 2 + 1  
        conv2_output_size = (conv1_output_size - 3) // 2 + 1 
        
        self.flatten = nn.Flatten()
        
        self.denseblock = nn.Sequential(
            nn.Linear(32 * conv2_output_size, 40),
            nn.ReLU(inplace=True),
            nn.Dropout(p=0.25),
        )
        
        self.out = nn.Linear(40, in_channels)
